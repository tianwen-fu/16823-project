...
class OpenCVPlayback(EventLoopHandler):
    ...
    def _process_data(self, data: DepthData) -> bool:
        ...
        gray_mapped, low, high = self._gray_map(gray_16u)
        ...
        found, corners = cv2.findChessboardCorners(gray_mapped, pattern_dims)
        title_appendix = ""
        if found:
            cv2.drawChessboardCorners(gray_mapped, pattern_dims, corners, found)
        ...
        if key == -1:
            ...
            if found or chr(key) == "f":
                ...
                self._gray_mapping_params.append((low, high))
            return True
        else:
